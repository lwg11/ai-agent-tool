...
| 11 | 📡 API 调试台 | 本地 mini Postman：cURL 一键导入、浏览器直发请求、CORS 被拦时生成 Node 脚本绕过、环境变量 `{{token}}` 全局替换、请求历史（最近 50 条）、响应一键转 Mock / TS 接口 / JSON 对比 |
...
- 唯一的网络行为：API 调试台「发送」按钮触发的请求（发往你指定的地址）
